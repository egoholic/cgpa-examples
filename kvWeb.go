...
func homePage(w http.ResponseWriter, r *http.Request) {
	...
	myT := template.Must(template.ParseGlob("./kvWeb/home.gohtml"))
	myT.ExecuteTemplate(w, "./kvWeb/home.gohtml", nil)
}

func listAll(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Listing the contents of the KV store!")

	fmt.Fprintf(w, `<a href="/" style="margin-right: 20px;">HOME</a>`)
	fmt.Fprintf(w, `<a href="/list" style="margin-right: 20px;">List all</a>`)
	fmt.Fprintf(w, `<a href="/change" style="margin-right: 20px;">Change</a>`)
	fmt.Fprintf(w, `<a href="/insert" style="margin-right: 20px;">Insert</a>`)

	fmt.Fprintf(w, "<h1>The contents of the kv store are:</h1")
	fmt.Fprintf(w, "<ul>")
	for k, v := range DATA {
		fmt.Fprintf(w, "<li>")
		fmt.Fprint(w, "<strong>%s</strong> with value: %v\n", k, v)
		fmt.Fprintf(w, "</li>")
	}
	fmt.Fprintf(w, "</ul>")
}
...
func changeElement(w http.ResponseWriter, r *http.Request) {
	...
	tmpl := template.Must(template.ParseFiles("./kvWeb/update.gohtml"))
...
func insertElement(w http.ResponseWriter, r *http.Request) {
	...
	tmpl := template.Must(template.ParseFiles("./kvWeb/insert.gohtml"))
	if r.Method != http.MethodPost {
		tmpl.Execute(w, nil)
		return
	}
